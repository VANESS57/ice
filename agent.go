...
func (a *Agent) handleInbound(m *stun.Message, local Candidate, remote net.Addr) { //nolint:gocognit
	...
	if m.Type.Class == stun.ClassSuccessResponse {
		...
		if remoteCandidate == nil {
			...
			srflxCandidateConfig := CandidateServerReflexiveConfig{
			...
			srflxCandidate, err := NewCandidateServerReflexive(&srflxCandidateConfig)

			if err != nil {
				a.log.Errorf("Failed to create new remote srflx candidate (%s)", err)
				return
			}
			remoteCandidate = srflxCandidate

			a.log.Debugf("adding a new server-reflexive candidate: %s ", remote)
